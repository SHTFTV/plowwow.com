...
// (public/…) and meets minimum social-crawler dimensions.
import { describe, it, expect } from "vitest";
import { existsSync } from "node:fs";
import { resolve } from "node:path";
import { collectRoutes, BASE_URL } from "../../scripts/routes";
import { readImageSize } from "./helpers/image-size";
...
describe("og:image reachability and dimensions", () => {
  ...
  it("every og:image meets social-crawler minimum dimensions", () => {
    ...
    for (const r of routes) {
      ...
      if (!size) {
        bad.push({ path: r.path, ogImage: r.ogImage!, reason: "unreadable image" });
        continue;
      }
