        ...
        <div className="flex items-center gap-2 mb-3">
          <Popover open={saveOpen} onOpenChange={setSaveOpen}>
            <PopoverTrigger asChild>
              <button
                disabled={!query.trim() && filter === "closures"}
                className="inline-flex items-center gap-1 rounded-md border border-border/60 bg-background/70 px-2 py-1 text-xs font-medium text-foreground hover:bg-muted transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              >
            ...
            <PopoverContent side="bottom" align="start" className="w-64 p-3">
              <p className="text-xs font-semibold text-foreground mb-2">Save this search</p>
              <p className="text-[11px] text-muted-foreground mb-2">
                Query: <span className="font-medium text-foreground">{query.trim() || "—"}</span> · Filter:{" "}
              ...
              <input
                ...
                className="w-full rounded-md border border-border/60 bg-background px-2 py-1 text-xs mb-2 focus:outline-none focus:ring-2 focus:ring-primary/30"
              />
              <button
                onClick={handleSaveSearch}
                className="w-full rounded-md bg-primary text-primary-foreground text-xs font-semibold py-1.5 hover:bg-primary/90 transition-colors"
              >
                Save search
              </button>
            </PopoverContent>
          ...
          <Popover open={savedListOpen} onOpenChange={setSavedListOpen}>
            <PopoverTrigger asChild>
              <button className="inline-flex items-center gap-1 rounded-md border border-border/60 bg-background/70 px-2 py-1 text-xs font-medium text-foreground hover:bg-muted transition-colors">
                <Bookmark className="w-3 h-3" />
                Saved
                {savedSearches.length > 0 && (
                  <span className="ml-0.5 inline-flex items-center justify-center rounded-full bg-primary/10 text-primary text-[10px] font-semibold px-1.5 min-w-[18px]">
            ...
            <PopoverContent side="bottom" align="start" className="w-72 p-2">
              <p className="text-xs font-semibold text-foreground px-1.5 py-1">Saved searches</p>
              {savedSearches.length === 0 ? (
                <p className="text-[11px] text-muted-foreground px-1.5 py-2">
                  No saved searches yet. Type a query or pick a filter, then click Save.
                </p>
              ) : (
